...
try
{
	...
	function checkParameter($param)
	{
		...
		{
			echo "<p>Missing <code>" . $param . "</code> parameter in query string. <br>Syntax is: <code>?adminAgreementNumber=<i>ADMINAGREEMENT</i>&adminUserID=<i>ADMINUSER</i>&adminUserPassword=<i>PASSWORD</i>&clientAgreementNo=<i>CLIENTAGREEMENT</i></code></p> ";

			exit(0);
		}
	}

	checkParameter("adminAgreementNumber");
	checkParameter("adminUserID");
	checkParameter("adminUserPassword");
	checkParameter("clientAgreementNo");
	$me = $_SERVER['PHP_SELF'];
	...
	$client->ConnectAsAdministrator(
		array(
			'adminAgreementNumber' 	=> $_REQUEST['adminAgreementNumber'],
			'adminUserID'        		=> $_REQUEST['adminUserID'],
			'adminUserPassword'     => $_REQUEST['adminUserPassword'],
			'clientAgreementNo'     => $_REQUEST['clientAgreementNo']
		)
	);
...
?>
	...
	<?php foreach ($debtorDataObjects as $i => $debtorData) : ?>
		<tr class="<?php if($i % 2 == 0) echo 'even_row'; else echo 'odd_row' ?>">
			<form action="<?php echo $me . "?adminAgreementNumber=" . $_REQUEST['adminAgreementNumber'] . "&adminUserID=" . $_REQUEST['adminUserID'] . "&adminUserPassword=" . $_REQUEST['adminUserPassword'] . "&clientAgreementNo=" . $_REQUEST['clientAgreementNo'];?>" method="post">
